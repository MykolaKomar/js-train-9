...
function findLastEvenNumber(arr) {
  ...
  return result || "'Парних чисел не знайдено'";
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const arrSmall = arr.slice(startIdx);
...
function getPositiveArrayValues(arr) {
  ...
  const arrResult = Array.from(iter);
  // За допомогою методу filter() отримуємо масив лише з додатніми значеннями.
  const result = arrResult.filter((elem) => elem > 0);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrNew = arr.splice(startIdx);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const arrMap = arrFlat.map((elem) => elem ** 2);
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  const result = arrMap.filter((elem) => elem > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = arrSlice.reduceRight((elem1, elem2) => elem1 + elem2, 0);
  // За допомогою методу forEach() виводимо кожен елемент з останніх n в консоль.
  arrSlice.forEach((n) => console.log(n));
...
function wordLengths(arr) {
  ...
  const arrNew = arr.map((elem) => elem.length);
...
function calculateTotalPrice(arr) {
  ...
  return arr.reduce((totalSum, product) => totalSum + product.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    if (arr[i] < 0) {
      return i; // Повертаємо індекс першого знайденого від'ємного числа
    }
  }

  // Якщо від'ємне число не знайдено, повертаємо -1
  return -1;
}
